...
def plot_scalar(ax, scalar: np.ndarray) -> mpl.image.AxesImage:
    ...
    """
    im = ax.imshow(scalar, cmap='inferno')
    cleanup_axes(ax)
    return im
...
def plot_scalar_sequence_comparison(init_field, ground_truth, prediction, fontsize=37, text_loc=(-10, 64)):
    ...
    # print("in viz: init_field", init_field.shape)
    im1 = plot_scalar(ax1, init_field[-1, ...])  # np.transpose(, (1, 2, 0))
    ...
    for i in range(n_timesteps):
        ...
        im1 = plot_scalar(ax, ground_truth[i, ...])  # np.transpose(, (1, 2, 0))

    # Plot the prediction sequence
    for i in range(n_timesteps):
        ax = fig.add_subplot(gs[1, i + 1])
        im2 = plot_scalar(ax, prediction[i, ...])  # np.transpose(, (1, 2, 0))
        if i == 0:
            ax.text(
    ...
    for i in range(n_timesteps):
        ...
        im3 = plot_scalar(ax, err[i, ...])  # np.transpose(, (1, 2, 0))
        if i == 0:
