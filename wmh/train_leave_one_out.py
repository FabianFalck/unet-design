...
from keras.preprocessing.image import apply_affine_transform, transform_matrix_offset_center
import warnings
...
def augmentation(x_0, x_1, y):
    ...
    shear = np.random.uniform(0, 360)
    zx, zy = np.random.uniform(.9, 1.1, 2)
    x_0 = apply_affine_transform(x_0[..., np.newaxis], theta=theta, shear=shear, zx=zx, zy=zy, row_axis=0, col_axis=1, channel_axis=2)
    x_1 = apply_affine_transform(x_1[..., np.newaxis], theta=theta, shear=shear, zx=zx, zy=zy, row_axis=0, col_axis=1, channel_axis=2)
    y = apply_affine_transform(y[..., np.newaxis], theta=theta, shear=shear, zx=zx, zy=zy, row_axis=0, col_axis=1, channel_axis=2)

    return x_0[..., 0], x_1[..., 0], y[..., 0]

#train single model on the training set
def train_leave_one_out(images, masks, patient=0, flair=True, t1=True, full=True, first5=True, aug=False, verbose=False):
    if full:
        if patient < 40:
...
def main():
    ...
    images = np.load(os.path.join(data_preprocess_path, 'images_three_datasets_sorted.npy'))
    masks = np.load(os.path.join(data_preprocess_path, 'masks_three_datasets_sorted.npy'))
    patient_num  = 60
