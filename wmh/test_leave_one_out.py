...
def Utrecht_postprocessing(FLAIR_array, pred):
    ...
    original_pred[int(num_selected_slice-start_slice-1):int(num_selected_slice-1), :, :] = 0
...
def test_leave_one_out(patient=0, flair=True, t1=True, full=True, first5=True, aug=True, verbose=False):
    ...
    filename_testImage = os.path.join(dir + '/wmh.nii.gz')  # see Manual reference standard --> contains 3 labels, the first two being background (negative) vs. WMH (positive) 
